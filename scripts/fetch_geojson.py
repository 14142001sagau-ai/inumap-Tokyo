...
STATIONS = [
 ...
 {'id':'edogawa','name':'江戸川','line':'京成本線','fl':2},
 {'id':'keiseikoiwa','name':'京成小岩','line':'京成本線','fl':2},
]
RADIUS = 1000
...
def get_coord(name):
 ...
 try:
  ...
  if els: return els[0]['lat'],els[0]['lon']
 except: pass
 return None,None
def get_fac(lat,lng):
...
for st in STATIONS:
 ...
 results.append({'id':st['id'],'name':st['name'],'line':st['line'],'lat':lat,'lng':lng,'walk':walk,'housing':HOUSING.get(st['id'],65),'medical':medical,'mobility':mobility,'community':COMMUNITY.get(st['id'],68),'safety':SAFETY.get(st['id'],70),'fl':st['fl'],'parks':fac['parks'],'vets':fac['vets'],'dogruns':fac['dogruns'],'carshares':fac['carshares']})
os.makedirs('data',exist_ok=True)
with open('data/stations.json','w',encoding='utf-8') as f: json.dump(results,f,ensure_ascii=False,indent=2)
print('✅ '+str(len(results))+'駅完了')
