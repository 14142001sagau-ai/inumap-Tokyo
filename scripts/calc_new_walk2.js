// Accurate new blended formula impact calculator
// Uses actual WALK_STATION base values from fetch_geojson.py

const fs = require('fs');
const stations = JSON.parse(fs.readFileSync('C:/inumap-repo2/data/stations.json','utf8'));

// Actual WALK_STATION values from fetch_geojson.py (lines ~1361+)
const WALK_STATION = {
  '西永福':76,'永福町':73,'祖師ヶ谷大蔵':57,'喜多見':57,
  '上板橋':76,'氷川台':70,'新高島平':67,'西高島平':60,
  '地下鉄成増':41,'成増':42,'沼袋':78,'野方':62,
  '代々木':57,'参宮橋':74,'明治神宮前':58,'駒沢大学':72,
  '石神井公園':62,'大泉学園':48,'光が丘':85,
  '吉祥寺':65,'武蔵小金井':55,'花小金井':48,
};

// STATION_OVERRIDE walk_base values
const OVERRIDE = {
  '光が丘':85,'石神井公園':69,'地下鉄赤塚':62,
};
...
console.log('駅名 | base | 現walk | 最近公園(距離m) | bonus | 旧式→新式 | 変化');
console.log('------------------------------------------------------------------------');
...
for (const st of stations) {
  ...
  const isOverride = !!OVERRIDE[st.name];
  const overrideBase = OVERRIDE[st.name] || base;

  const nearLarge = LARGE_PARKS.filter(p => minParkDist(st.lat, st.lng, p) <= 1600);
  let bonus = 0, nearestParkName = '-', nearestDist = 99999;
  for (const p of LARGE_PARKS) {
    const d = minParkDist(st.lat, st.lng, p);
    if (d < nearestDist) { nearestDist = d; nearestParkName = p.name; }
  }
  ...
  // Estimate current osm_score by back-calculating from current walk
  // walk = blended(osm_score, base) => osm_score = (walk - base*0.3) / 0.7
  const currentWalk = st.walk;
  const estimatedOsm = Math.round((currentWalk - base * 0.3) / 0.7);

  // For new formula, we use: osm_score = park_score + bonus (estimated)
  // We assume park_score stays the same but bonus may change
  // The main change is the blended formula itself
  const osmWithBonus = Math.max(estimatedOsm, bonus); // bonus is the floor for near-park stations

  const oldW = oldBlended(osmWithBonus, base);
  const newW = newBlended(osmWithBonus, base);

  // Apply STATION_OVERRIDE max()
  const finalOld = isOverride ? Math.max(oldW, overrideBase) : oldW;
  const finalNew = isOverride ? Math.max(newW, overrideBase) : newW;

  const diff = finalNew - currentWalk;
  const marker = diff > 0 ? `↑+${diff}` : diff < 0 ? `↓${diff}` : '=';

  const distM = Math.round(nearestDist);
  console.log(`${st.name}: base=${base}, walk=${currentWalk}, 最近=${nearestParkName.slice(0,8)}(${distM}m), bonus=${bonus}, osm推定=${osmWithBonus}, 旧→${finalOld}, 新→${finalNew} ${marker}`);
}
