  ...
  return React.createElement('div',{style:{display:'flex',flexDirection:'column',height:'100dvh',overflow:'hidden'}},
    React.createElement('div',{style:{background:'#fff',borderBottom:'1px solid #dde4dd',padding:'5px 10px',flexShrink:0}},
      ...
      React.createElement('div',{style:{display:'flex',height:28,borderRadius:8,overflow:'hidden'}},
        AXES.map(ax=>React.createElement('div',{key:ax.k,style:{flex:w[ax.k],background:ax.co,display:'flex',alignItems:'center',justifyContent:'center',minWidth:0}},
          w[ax.k]>=12&&React.createElement('span',{style:{fontSize:8,color:'#fff',fontWeight:700,whiteSpace:'nowrap',padding:'0 3px'}},ax.lb.split(' ')[0]+' '+w[ax.k]+'%')
        ))
      )
    ),
    React.createElement('div',{style:{flex:1,position:'relative',overflow:'hidden'}},
      ...
      React.createElement('div',{style:{position:'absolute',top:8,left:8,background:'rgba(255,255,255,0.92)',borderRadius:10,padding:'8px 11px',zIndex:1000,boxShadow:'0 2px 8px rgba(0,0,0,0.15)'}},
        React.createElement('div',{style:{fontSize:8,fontWeight:700,color:'#2e7d32',marginBottom:5}},'SCORE'),
        [{d:66,l:'S 65+'},{d:60,l:'A 58-64'},{d:53,l:'B 50-57'},{d:45,l:'C 42-49'},{d:38,l:'D ~41'}].map(item=>{const s=gs(item.d);return React.createElement('div',{key:item.l,style:{display:'flex',alignItems:'center',gap:5,marginBottom:3}},
          React.createElement('div',{style:{width:10,height:10,borderRadius:'50%',background:s.c}}),
          React.createElement('span',{style:{fontSize:9.5,color:'#3a5a3a'}},item.l)
        );}),
        React.createElement('div',{style:{borderTop:'1px solid #dde4dd',marginTop:7,paddingTop:7,display:'flex',justifyContent:'center'}},
          React.createElement('button',{onClick:()=>setFlood(f=>!f),style:{width:42,height:42,borderRadius:'50%',border:'2px solid '+(flood?'#d32f2f':'#ccc'),background:flood?'rgba(211,47,47,0.12)':'rgba(200,200,200,0.18)',color:flood?'#d32f2f':'#888',fontSize:18,cursor:'pointer',display:'flex',alignItems:'center',justifyContent:'center',boxShadow:flood?'0 0 0 2px #d32f2f44':'none'}},'\uD83C\uDF0A')
        ),
        React.createElement('div',{style:{fontSize:7,color:flood?'#d32f2f':'#aaa',textAlign:'center',marginTop:2,fontWeight:700}},flood?'ON':'\u6D2A\u6C34'),
        React.createElement('div',{style:{borderTop:'1px solid #dde4dd',marginTop:7,paddingTop:7}}),
        React.createElement('div',{style:{fontSize:7.5,fontWeight:700,color:'#2e7d32',marginBottom:5}},'TOP 5'),
        sorted.slice(0,5).map((d,i)=>{const s=gs(d.dev);const iS=d.id===sel;return React.createElement('div',{key:d.id,onClick:()=>setSel(iS?null:d.id),style:{display:'flex',alignItems:'center',gap:4,padding:'3px 5px',borderRadius:6,cursor:'pointer',marginBottom:2,background:iS?s.c+'18':'transparent',border:'1px solid '+(iS?s.c:'transparent')}},
          React.createElement('span',{style:{fontSize:7.5,color:'#bbb',width:8}},i+1),
          React.createElement('span',{style:{width:15,height:15,borderRadius:3,background:s.c,display:'flex',alignItems:'center',justifyContent:'center',fontSize:7.5,fontWeight:900,color:'#fff',flexShrink:0}},s.b),
          React.createElement('span',{style:{fontSize:9,fontWeight:700,color:'#2a3a2a',whiteSpace:'nowrap'}},d.name),
          React.createElement('span',{style:{fontSize:10.5,fontWeight:900,color:s.c,marginLeft:'auto'}},d.dev)
        );})
      ),
