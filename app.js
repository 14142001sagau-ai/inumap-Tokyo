...
//   ランク: S(75+) A(65-74) B(55-64) C(45-54) D(35-44)
// housing: 地価ベーススコア（45〜75）低=安い=高スコア
// medical: 動物病院アクセススコア（30〜90）
// walk: 公園・ドッグランスコア（30〜90）
...
// 大型犬ボーナス（AY/BY: ドッグラン・大型公園を特に重視）
const largeDogBonus=(d)=>{
  let b=0;
  if(d.dogruns&&d.dogruns.length>0) b+=6;  // ドッグランあり: 最重要
  if(d.walk>=75) b+=4;                      // 広大な公園環境
  else if(d.walk>=65) b+=2;
  return b;
};

const PENALTY={
  // ── ファミリー系 ──────────────────────────
  // BX: ファミリー×小型犬 → 治安最重視、高家賃もやや考慮
  BX:(d)=>safetyPen(d,true,false)+(d.housing<50?-3:0),
  // BY: ファミリー×大型犬 → 治安+大型公園・ドッグランボーナス
  BY:(d)=>safetyPen(d,true,false)+largeDogBonus(d)+(d.housing<48?-5:0),
  // BZ: ファミリー×シニア犬 → 治安+医療の両方が必須
  BZ:(d)=>safetyPen(d,true,false)+(d.medical<45?-8:0),
  // ── 単身系 ───────────────────────────────
  // AX: 単身×小型犬 → 女性一人暮らし想定、治安はファミリー並みに重視
  //     高家賃は許容（DINKs展開時に再設計）
  AX:(d)=>safetyPen(d,false,true),
  // AY: 単身×大型犬 → 大型公園・ドッグランボーナス+治安
  AY:(d)=>safetyPen(d,false,false)+largeDogBonus(d)+(d.housing<48?-5:0),
  // AZ: 単身×シニア犬 → 医療アクセス最重要+治安
  AZ:(d)=>safetyPen(d,false,false)+(d.medical<45?-8:0),
};
  ...
  return React.createElement('div',{style:{display:'flex',flexDirection:'column',height:'100dvh',overflow:'hidden'}},
    ...
    React.createElement('div',{style:{flex:1,position:'relative',overflow:'hidden'}},
      ...
      React.createElement('div',{style:{position:'absolute',top:8,left:8,background:'rgba(255,255,255,0.92)',borderRadius:8,padding:'5px 8px',zIndex:1000,boxShadow:'0 2px 8px rgba(0,0,0,0.15)'}},
        ...
        [{d:72,l:'SS 70+'},{d:66,l:'S 65-69'},{d:60,l:'A 58-64'},{d:53,l:'B 50-57'},{d:45,l:'C 42-49'},{d:38,l:'D 35-41'},{d:30,l:'E ~34'}].map(item=>{const s=gs(item.d);return React.createElement('div',{key:item.l,style:{display:'flex',alignItems:'center',gap:4,marginBottom:2,paddingLeft:20}},
